import { Liquidity, MARKET_STATE_LAYOUT_V2, MARKET_STATE_LAYOUT_V3, SPL_ACCOUNT_LAYOUT } from '@raydium-io/raydium-sdk';
...
async function getSerumMarketForToken(tokenMint, commitment = "confirmed") {
    const { span } = MARKET_STATE_LAYOUT_V3;
    const filters = [{ dataSize: span },
        {
            memcmp: {
...
const getPoolKeys = async (addr) => {
    var _a;
    if (mintDataCache.has(addr) && 'poolKeys' in mintDataCache.get(addr)) {
        return (_a = mintDataCache.get(addr)) === null || _a === void 0 ? void 0 : _a.poolKeys;
    }
    ...
    const marketData = MARKET_STATE_LAYOUT_V3.decode(market.account.data);
    ...
    if (mintDataCache.has(addr)) {
        ...
        curr = { priceCache: { closeAccountListeners: [] }, poolKeys: poolKeys };
    }
...
async function getTokenPriceRaydium(poolKeys) {
    try {
        if (poolKeys.quoteMint.toString() == NATIVE_MINT.toString()) {
...
function isCacheStale(updatedDate, price_cache_ttl = 2) {
    if (!updatedDate)
        return true;
    const now = new Date();
...
async function getCachedTokenPrice(tokenAddr, poolKeys) {
...
async function listenToTokenRealTime(tokenAddr, addressToWatch) {
